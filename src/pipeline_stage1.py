...
class IngestionEngine:
    ...
    @staticmethod  # Changed to staticmethod to prevent 'classmethod is not callable' bugs
    def clean_crm_data(filepath: str) -> pd.DataFrame:
        """
        ...
        for target_key, keywords in mappings.items():
            # Call find_column directly via class reference
            found_col = IngestionEngine.find_column(cols, keywords)
            
            # Dynamic Fallback: if 'category' is not found for Ticket Type, search for 'type'
            if not found_col and target_key == 'Ticket Type':
                found_col = IngestionEngine.find_column(cols, ['type'])
        ...
        id_col = IngestionEngine.find_column(cols, ['id'])
        if id_col:
...
class OperationalLatencyEvaluator:
    ...
    @staticmethod  # Changed to staticmethod to prevent compiler bugs
    def assign_operational_scores(df: pd.DataFrame) -> pd.Series:
        """
        Bins Z-scores into normalized severity categories 1 to 4.
        """
        z_scores = OperationalLatencyEvaluator.compute_z_scores(df)
